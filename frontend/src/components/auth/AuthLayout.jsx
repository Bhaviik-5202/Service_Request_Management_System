...
export function LoginForm() {
  ...
  const onSubmit = async (data) => {
    setIsLoading(true);
    const success = await signIn(data.email, data.password, remember);
    setIsLoading(false);
...
export function SignupForm() {
  ...
  const [deptList, setDeptList] = useState(["IT", "Operations", "Sales", "Maintenance"]);
  ...
  useEffect(() => {
    async function loadDepts() {
      try {
        ...
        if (res && res.length > 0) {
          const names = res.map((d) => d.departmentName || d.name);
          setDeptList(names);
