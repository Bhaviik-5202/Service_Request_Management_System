...
import {
  ...
  updateRequestType,
  deleteRequestType,
  getRequestTypeTechnicianMappings,
  createRequestTypeTechnicianMapping,
  deleteRequestTypeTechnicianMapping,
  getUsers,
} from "@/services/api";
...
function StatusesManager() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const apiStatuses = await getServiceRequestStatuses();
      const mapped = (apiStatuses || []).map((s) => ({
      ...
      toast.error("Failed to load statuses from server.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleConfirmDelete = async () => {
    try {
      await deleteServiceRequestStatus(deletingId);
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      const payload = {
        statusName: formData.name,
        description: formData.description,
        colorClass: formData.color,
        isActive: formData.isActive ?? true,
      };

      if (editingItem) {
        await updateServiceRequestStatus(editingItem.id, payload);
        toast.success("Status updated successfully!");
      } else {
        await createServiceRequestStatus(payload);
...
function DepartmentsManager() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const apiDepts = await getDepartments();
      const mapped = (apiDepts || []).map((d) => ({
...
function PersonnelManager() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [apiPersonnel, apiUsers, apiDepts] = await Promise.all([
        getDepartmentPersonnel().catch(() => []),
        getUsers().catch(() => []),
        getDepartments().catch(() => []),
      ]);

      setUserList(apiUsers || []);
      setDeptList(apiDepts || []);

      const mapped = (apiPersonnel || []).map((dp) => {
        const u = (apiUsers || []).find((x) => x.userId === dp.userId || String(x.userId) === String(dp.userId));
        const d = (apiDepts || []).find((x) => x.departmentId === dp.departmentId || String(x.departmentId) === String(dp.departmentId));
        return {
          id: String(dp.departmentPersonnelId),
          userId: String(dp.userId),
          departmentId: String(dp.departmentId),
          userName: u ? (u.fullName || u.name) : (dp.user?.fullName || "Unknown User"),
          userEmail: u ? u.email : (dp.user?.email || ""),
          deptName: d ? d.departmentName : (dp.department?.departmentName || "Unknown Department"),
          isHOD: dp.isHOD ?? false,
          isActive: dp.isActive ?? true,
        };
      });
      setData(mapped);
    } catch (err) {
      toast.error("Failed to load personnel mappings.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleConfirmDelete = async () => {
    try {
      await deleteDepartmentPersonnel(deletingId);
      toast.success("Personnel mapping removed successfully!");
      await loadData();
    } catch (err) {
      toast.error(err.message || "Failed to remove personnel mapping.");
    } finally {
      setIsDeleteOpen(false);
    }
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      await createDepartmentPersonnel(payload);
      toast.success("Personnel mapped successfully!");
      setIsModalOpen(false);
      await loadData();
    } catch (err) {
      toast.error(err.message || "Failed to save personnel mapping.");
    }
  };
...
function ServiceTypesManager() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const apiTypes = await getServiceTypes();
      const mapped = (apiTypes || []).map((st) => ({
  ...
  const handleConfirmDelete = async () => {
    try {
      await deleteServiceType(deletingId);
...
function RequestTypesManager() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [apiReqTypes, apiServTypes] = await Promise.all([
      ...
      const mapped = (apiReqTypes || []).map((rt) => {
        const st = (apiServTypes || []).find(
          (x) => x.serviceTypeId === rt.serviceTypeId || String(x.serviceTypeId) === String(rt.serviceTypeId)
        );
        return {
          ...
          serviceTypeName: st ? st.serviceTypeName : (rt.serviceType?.serviceTypeName || "Unknown Service Type"),
  ...
  const handleConfirmDelete = async () => {
    try {
      await deleteRequestType(deletingId);
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      ...
      if (editingItem) {
        await updateRequestType(editingItem.id, payload);
...
function MappingsManager() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [apiMappings, apiReqTypes, apiServTypes, apiPersonnel, apiUsers, apiDepts] = await Promise.all([
        getRequestTypeTechnicianMappings().catch(() => []),
        getRequestTypes().catch(() => []),
        getServiceTypes().catch(() => []),
        getDepartmentPersonnel().catch(() => []),
        getUsers().catch(() => []),
      ...
      setReqTypeList(apiReqTypes || []);

      const techs = (apiPersonnel || []).map((dp) => {
        const u = (apiUsers || []).find((x) => x.userId === dp.userId || String(x.userId) === String(dp.userId));
        const d = (apiDepts || []).find((x) => x.departmentId === dp.departmentId || String(x.departmentId) === String(dp.departmentId));
        return {
          id: String(dp.departmentPersonnelId),
          name: u ? (u.fullName || u.name) : "Unknown Staff",
          email: u ? u.email : "",
          dept: d ? d.departmentName : "",
        };
      });
      setPersonnelList(techs);

      const mapped = (apiMappings || []).map((m) => {
        const rt = (apiReqTypes || []).find((x) => x.requestTypeId === m.requestTypeId || String(x.requestTypeId) === String(m.requestTypeId));
        const st = rt ? (apiServTypes || []).find((x) => x.serviceTypeId === rt.serviceTypeId || String(x.serviceTypeId) === String(rt.serviceTypeId)) : null;

        const dp = (apiPersonnel || []).find((x) => x.departmentPersonnelId === m.departmentPersonnelId || String(x.departmentPersonnelId) === String(m.departmentPersonnelId));
        const u = dp ? (apiUsers || []).find((x) => x.userId === dp.userId || String(x.userId) === String(dp.userId)) : null;
        const d = dp ? (apiDepts || []).find((x) => x.departmentId === dp.departmentId || String(x.departmentId) === String(dp.departmentId)) : null;

        return {
          id: String(m.mappingId),
          requestTypeId: String(m.requestTypeId),
          personId: String(m.departmentPersonnelId),
          requestTypeName: rt ? rt.requestTypeName : (m.requestType?.requestTypeName || "Unknown Request Type"),
          serviceTypeName: st ? st.serviceTypeName : "",
          personName: u ? (u.fullName || u.name) : "Unknown Staff",
          personEmail: u ? u.email : "",
          personDept: d ? d.departmentName : "",
          isActive: m.isActive ?? true,
        };
      });
      setData(mapped);
    } catch (err) {
      toast.error("Failed to load technician assignment mappings.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleConfirmDelete = async () => {
    try {
      await deleteRequestTypeTechnicianMapping(deletingId);
      toast.success("Assignment mapping deleted successfully!");
      await loadData();
    } catch (err) {
      toast.error(err.message || "Failed to delete assignment mapping.");
    } finally {
      setIsDeleteOpen(false);
    }
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      const payload = {
        ...
        departmentPersonnelId: Number(selectedPerson),
        isActive,
      };

      await createRequestTypeTechnicianMapping(payload);
      toast.success("Assignment mapping created successfully!");
      setIsModalOpen(false);
      await loadData();
    } catch (err) {
      toast.error(err.message || "Failed to save assignment mapping.");
    }
  };
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          >
            ...
            <SelectContent>
              ...
              {reqTypeList
                .filter((rt) => rt.isActive ?? true)
                .map((rt) => (
                  <SelectItem key={rt.requestTypeId} value={String(rt.requestTypeId)}>
                    {rt.requestTypeName}
                  </SelectItem>
                ))}
            </SelectContent>
