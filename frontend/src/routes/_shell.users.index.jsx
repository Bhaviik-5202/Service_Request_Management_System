...
function UsersPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const mapped = (apiUsers || []).map((u) => {
        let roleName = u.roleName || u.role?.roleName;
        if (!roleName && u.roleId && (apiRoles || []).length > 0) {
          const r = apiRoles.find((role) => role.roleId === u.roleId);
          if (r) roleName = r.roleName;
        }
        if (!roleName) roleName = "Requestor";

        let deptName = u.departmentName || u.department?.departmentName;
        if (!deptName && u.departmentId && (apiDepts || []).length > 0) {
          const d = apiDepts.find((dept) => dept.departmentId === u.departmentId);
        ...
        return {
          ...
          joined: u.joinedDate ? new Date(u.joinedDate).toISOString().split("T")[0] : new Date().toISOString().split("T")[0],
        };
      });
      setData(mapped);
    } catch (err) {
  ...
  const handleOpenAdd = () => {
    ...
    setSelectedDept(departmentsList[0]?.departmentName || "IT");
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      let roleId = 4;
      const foundRole = rolesList.find((r) => r.roleName === selectedRole);
      if (foundRole) roleId = foundRole.roleId;

      let departmentId = null;
      const foundDept = departmentsList.find((d) => (d.departmentName || d.name) === selectedDept);
      if (foundDept) departmentId = foundDept.departmentId;

      const payload = {
        employeeId: editingItem?.employeeId || `EMP-${Date.now().toString().slice(-4)}`,
        fullName: formData.name,
        email: formData.email,
        phone: formData.phone,
        passwordHash: editingItem?.passwordHash || "DefaultPass123!",
        roleId: roleId,
      ...
      render: (row) => (
        <span
          className={cn(
            ...
            roleStyles[row.role] || "bg-muted text-muted-foreground ring-border",
          )}
        >
          {row.role}
        </span>
              ...
              <Select value={selectedDept} onValueChange={setSelectedDept}>
                ...
                <SelectContent>
                  {departmentsList.map((d) => (
                    <SelectItem key={d.departmentId} value={d.departmentName}>
                      {d.departmentName}
                    </SelectItem>
                  ))}
                </SelectContent>
