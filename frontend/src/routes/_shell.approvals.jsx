...
import { Check, X, Clock, Search, Inbox, Loader2 } from "lucide-react";
...
function ApprovalsPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [apiApprovals, apiRequests] = await Promise.all([
        getApprovals().catch(() => []),
        getServiceRequests().catch(() => []),
      ]);

      const mapped = (apiApprovals || []).map((a) => {
        const sr = (apiRequests || []).find(
          (r) => r.requestId === a.requestId || String(r.requestId) === String(a.requestId)
        );
        ...
        return {
          ...
          requestNo: a.serviceRequest?.requestNumber || sr?.requestNumber || `SR-${a.requestId}`,
          title: a.serviceRequest?.title || sr?.title || "Service Request",
          requester: a.serviceRequest?.requesterUser?.fullName || sr?.requesterUser?.fullName || "Requester",
