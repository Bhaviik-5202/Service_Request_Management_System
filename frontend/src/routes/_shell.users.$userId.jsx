...
import { getUserById, getServiceRequests, getRoles, getDepartments } from "@/services/api";
import { cn } from "@/lib/utils";

export const Route = createFileRoute("/_shell/users/$userId")({
  head: ({ params }) => ({
    meta: [
      { title: "User Details — ServiceDesk" },
...
function UserProfile() {
  ...
  useEffect(() => {
    async function fetchUserData() {
      setLoading(true);
      try {
        const [apiUser, allRequests, apiRoles, apiDepts] = await Promise.all([
        ...
        let roleName = apiUser.roleName || apiUser.role?.roleName;
        if (!roleName && apiUser.roleId && apiRoles.length > 0) {
          const r = apiRoles.find((role) => role.roleId === apiUser.roleId);
          if (r) roleName = r.roleName;
        }
        if (!roleName) roleName = "Requestor";

        let deptName = apiUser.departmentName || apiUser.department?.departmentName;
        if (!deptName && apiUser.departmentId && apiDepts.length > 0) {
          const d = apiDepts.find((dept) => dept.departmentId === apiUser.departmentId);
          if (d) deptName = d.departmentName;
        }
        ...
        // Related requests
        const related = (allRequests || []).filter(
          (r) =>
            r.requesterUserId === Number(userId) ||
            r.assigneeUserId === Number(userId) ||
            r.requester === name ||
            r.assignee === name
        ).map((r) => ({
          id: String(r.requestId || r.id),
          no: r.requestNumber || r.no || `SR-${r.requestId}`,
          title: r.title,
          priority: r.priority || "Medium",
          status: r.status?.statusName || r.status || "Pending",
        })).slice(0, 5);

        const raisedCount = (allRequests || []).filter(
          (r) => r.requesterUserId === Number(userId) || r.requester === name
        ).length;

        const resolvedCount = (allRequests || []).filter(
          (r) => (r.assigneeUserId === Number(userId) || r.assignee === name) && (r.status?.statusName === "Closed" || r.status === "Closed" || r.status?.statusName === "Resolved" || r.status === "Resolved")
        ).length;
        ...
        setUser({
          ...
          joined: apiUser.joinedDate ? new Date(apiUser.joinedDate).toISOString().split("T")[0] : new Date().toISOString().split("T")[0],
          requestsRaised: raisedCount,
          requestsResolved: resolvedCount,
        });
        setUserRequests(related);
      } catch (err) {
        setNotFoundState(true);
      } finally {
        setLoading(false);
  ...
  if (loading) {
    return (
      <div className="py-20 text-center">
        ...
        <p className="mt-3 text-sm text-muted-foreground">Loading user profile…</p>
      </div>
    );
  }

  if (notFoundState || !user) {
    return (
      <div className="py-20 text-center">
